...
## Custom QML/command bar modules (third-party widgets)

For a custom bar widget that must survive shell restarts and show data from a
script, the **documented custom-module path is more reliable than a full
plugin** when the plugin registry rescan is flaky/unresponsive:

1. Put the QML at `~/.config/omarchy/bar/modules/<id>.qml` (an `Item` with
   `property var bar`, `property var moduleName`, `property var settings`, and
   `implicitWidth`/`implicitHeight`). It may import `Quickshell` and
   `Quickshell.Io` and use `Process { stdout: StdioCollector { waitForEnd: true;
   onStreamFinished: ... } }` with a `Timer` to poll (data read works from
   inside the shell process).
2. Reference it in `shell.json` bar layout: `{"id": "<id>", "type": "qml"}`.
   Hot-reloads; use `omarchy-shell shell rescanPlugins` (rc=0) + wait ~70s for
   the first timer tick before checking `omarchy-shell shell debugBarGeometry`.
3. Avoid `Qt.resolvedUrl()` for paths inside QML modules (returns a URL object
   that fails string ops in this runtime) — build paths from
   `Quickshell.env("HOME")` instead.
4. `Process.onStdoutData` does NOT exist in this Quickshell version — use the
   `StdioCollector` pattern.
5. Verify a widget is actually mounted with `omarchy-shell shell debugBarGeometry`
   (lists visible modules; invisible/unmounted widgets don't appear).

A full third-party *plugin* (manifest.json + kinds:[bar-widget]) is the
canonical way but requires the registry to scan `~/.config/omarchy/plugins/`;
if the shell's rescan is unresponsive ("omarchy-shell is not responding"),
the custom QML module route above still works.
