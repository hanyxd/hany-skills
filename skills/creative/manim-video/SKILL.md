...
### Manim CE v0.21 quirks
- `LineJointType` lives in `manim.constants` (not `manim.utils`) and its members are AUTO/ROUND/BEVEL/MITER — there is NO `JOINED`. Set it as an attribute, not a kwarg: `tick.joint_type = LineJointType.ROUND` (passing `join_style=` to `set_stroke()` raises TypeError).
- This version lists available scenes instead of accepting `all_scenes`; pass scene names explicitly to render.
- On a pip-less Arch host, install Manim via `uv venv` + `uv pip install manim` and keep cairo/pango from the system pacman packages — neither LaTeX nor texlive is required for Text/shape-only ("Explainer Boss") scenes.
